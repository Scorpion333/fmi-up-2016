...
int main()
{
	...
	// Symbols can be digits or other signs
	char mySymbol = '5';
	mySymbol = '!';
	mySymbol = ';';
	...
	// If we want more operations to be executed, we must put { }
	if(a > 9)
	{
	...
	myFirstBoolVariable = true;		// These variables have two possible values - true and false.
	bool x = false;
	...
	if (myFirstBoolVariable || 4 - 5 == 0)
		cout << "At least one of them are true.";	// Grammar is not necessary if we understand everything. :)
	else
	...
	// And finally, two useful double symbols:
	a++;		// The same as:   a = a + 1;
	a--;		// The same as:   a = a - 1;
